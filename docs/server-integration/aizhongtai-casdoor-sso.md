...
```powershell
$env:LZCLAW_SERVER_API_BASE_URL='http://127.0.0.1:8081'
...
这个顺序保证正式企业包可由 manifest 配置 API 地址，同时本地联调可以直接通过环境变量切到本机 AIZhongtai API，不受历史 app config 影响。
...
- `GET /api/workstation/models/pricing-catalog`
...
1. 启动 Casdoor: `http://127.0.0.1:8000`。
