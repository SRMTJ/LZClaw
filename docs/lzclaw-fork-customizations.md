...
| 本地工作目录品牌 | P0 | 新任务的默认工作目录直接使用用户主目录下的 `htmmai/project`；任务隔离目录使用 `.htmmai-tasks`，OpenClaw 工作区迁移备份使用 `.htmmai/migrations`，cron doctor 临时配置使用 `.htmmai-cron-doctor-openclaw.json`；不迁移也不读取旧的 `lobsterai` 目录 | `src/main/coworkStore.ts`、`src/main/libs/openclawAgentsMdIdentityMigration.ts`、`src/main/libs/openclawCronLegacyMigration.ts` |
...
| 业务中心 | P0 | 侧边栏在 MCP 下方显示“业务中心”；开发和生产构建均固定使用 `https://qiye.srmtj.com`，已登录时按服务端确认的角色进入 `/admin/` 或 `/employee/`，以复用对应路径作用域的 Portal Cookie，不能要求用户再次登录；切换菜单时只隐藏视图并保留滚动、表单和页面状态；只有持久化视图返回 `opened` 才向当前 IPC 调用报告打开成功 | `src/renderer/components/Sidebar.tsx`、`src/renderer/components/businessCenter/BusinessCenterView.tsx`、`src/main/libs/businessCenterInAppView.ts`、`src/shared/businessCenter/constants.ts` |
