...
<p align="center">
  ...
  <a href="https://x.com/LobsterAIYoudao"><img src="https://img.shields.io/badge/-000000?logo=x&logoColor=white" alt="在 X 关注海豚买买AI工作台" /></a>
  <a href="https://shared.ydstatic.com/market/souti/fihserChatWeb/online/2.0.7/dist/assets/wechat_group-B34qRm1G.png"><img src="https://img.shields.io/badge/-000000?logo=wechat&logoColor=white" alt="在微信关注海豚买买AI工作台" /></a>
  <br>
...
<h3 align="center"><a href="https://lobsterai.youdao.com/#/download-list"><ins>下载海豚买买AI工作台</ins></a></h3>

<p align="center">
  <img src="docs/res/mainpage_zh.png" alt="main page" />
</p>

海豚买买AI工作台是一个可以进入真实工作环境的桌面级 Agent：本地文件、终端命令、浏览器流程、文档、表格、幻灯片、IM 渠道、定时任务和项目工作区。

Cowork 是海豚买买AI工作台的产品与会话层，OpenClaw 是底层运行时和网关。这种分层让海豚买买AI工作台在桌面端负责本地持久化、权限、UI 状态、Artifacts、Agents、记忆和 IM 绑定，同时由 OpenClaw 执行 Agent 任务。

## 功能亮点

### 桌面级 Cowork 会话

围绕本地项目和文件执行长任务。海豚买买AI工作台会实时流式展示进度、保存会话历史、渲染工具输出，并在文件操作、终端命令、网络访问等敏感动作前请求用户审批。
...
海豚买买AI工作台在 `SKILLs/skills.config.json` 中配置了 28 个内置技能，包括 Web 搜索、Word 文档、Excel 表格、PowerPoint、PDF 处理、Remotion 视频生成、浏览器自动化、图片/视频生成、股票研究、内容写作、邮件、天气和技能创建等。

### MCP 服务

通过 Model Context Protocol 接入外部工具和数据源。海豚买买AI工作台会在本地保存用户配置的 MCP 服务，并将启用的服务同步到 OpenClaw。
...
<p align="center">
  <img src="docs/res/architecture_v2_zh.png" alt="海豚买买AI工作台架构" width="640">
</p>
...
- **OpenClaw 集成**：`openclawEngineManager`、`openclawConfigSync`、`openclawRuntimeAdapter` 和 `coworkEngineRouter` 将海豚买买AI工作台状态转换成 OpenClaw 运行时行为。
...
| `src/main/libs/openclawConfigSync.ts` | 将海豚买买AI工作台的 provider、model、agent、IM 绑定、skills、MCP 和工作区指令渲染为 OpenClaw 配置 |
