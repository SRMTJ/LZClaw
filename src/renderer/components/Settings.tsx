import { ChatBubbleLeftIcon, CpuChipIcon, EnvelopeIcon, GlobeAltIcon, InformationCircleIcon, SunIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const normalizeSettingsTab = (tab?: TabType): TabType =>
  tab === 'coworkAgentEngine' || tab === 'model' ? 'general' : tab ?? 'general';
...
const Settings: React.FC<SettingsProps> = ({ onClose, initialTab, notice, noticeI18nKey, noticeExtra, onUpdateFound, enterpriseConfig }) => {
  ...
  const sidebarTabs: { key: TabType; label: string; icon: React.ReactNode }[] = (() => {
    const allTabs = [
      ...
      // LZClaw customization: keep model tab hidden.
