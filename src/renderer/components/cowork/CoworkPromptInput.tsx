...
>((props, ref) => {
  ...
  const [showFolderRequiredWarning, setShowFolderRequiredWarning] =
    useState(false);
  ...
  const activeSkillIds = useSelector(
    (state: RootState) => state.skill.activeSkillIds,
  );
  ...
  useEffect(() => {
    const handleFocusInput = (event: Event) => {
      const detail = (event as CustomEvent<{ clear?: boolean; text?: string }>)
        .detail;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (imageAtts.length > 0) {
      console.log(
        "[CoworkPromptInput] handleSubmit: passing imageAtts to onSubmit",
        {
  ...
  const truncatePath = (path: string, maxLength = 30): string => {
    ...
    return (
      getCompactFolderName(path, maxLength) || i18nService.t("noFolderSelected")
    );
  };
        ...
        if (fileIsImage) {
          if (modelSupportsImage) {
            ...
            if (nativePath) {
              try {
                const result =
                  await window.electron.dialog.readFileAsDataUrl(nativePath);
      ...
      for (const filePath of result.paths) {
        if (isImagePath(filePath)) {
          if (modelSupportsImage) {
            try {
              const readResult =
                await window.electron.dialog.readFileAsDataUrl(filePath);
                ...
                {showFolderSelector && (
                  <>
                    ...
                    <FolderSelectorPopover
                      ...
                      anchorRef={
                        folderButtonRef as React.RefObject<HTMLElement>
                      }
                    />
                  </>
                )}
                {showModelSelector && !remoteManaged && (
                  <ModelSelector dropdownDirection="up" />
                )}
                {!remoteManaged && (
                  <button
                    type="button"
