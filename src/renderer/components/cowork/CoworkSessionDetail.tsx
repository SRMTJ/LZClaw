...
const buildRailItems = (
  ...
  for (let index = 0; index < turns.length; index += 1) {
    ...
    items.push({
      ...
      label: turn.userMessage ? getRailLabel(userContent, `Turn ${index + 1}`) : 'LZClaw',
      summary: assistantContent
        ? getRailLabel(assistantContent, 'LZClaw', COWORK_RAIL_TOOLTIP_PREVIEW_MAX_LENGTH)
        : '',
...
const composeExportCanvas = async (
  ...
  ctx.fillText('LZClaw — 全场景个人助理 Agent', textX, footerCenterY - taglineFontSize / 2 - 2);

  ctx.fillStyle = subtitleColor;
  ctx.font = `400 ${taglineFontSize}px ${fontStack}`;
  ctx.fillText('7×24 小时帮你干活的全场景个人助理，由LZClaw开发', textX, footerCenterY + brandFontSize / 2 + 3);
