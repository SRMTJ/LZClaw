...
if (!gotTheLock) {
  ...
  ipcMain.handle('auth:login', async (_event, { loginUrl }: { loginUrl?: string } = {}) => {
    ...
    try {
      ...
      const resolvedBaseUrl = requestedLoginUrl || await fetchLoginUrl();
      ...
      const returnTo = appendLoginParams(resolvedBaseUrl, {
        source: 'electron',
        electronLogin: 'success',
      });
      const finalUrl = appendLoginParams(resolvedBaseUrl, {
        source: 'electron',
