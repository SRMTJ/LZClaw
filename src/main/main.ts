...
const resolveInlineAttachmentDir = (cwd?: string): string => {
  ...
  return path.join(app.getPath('temp'), 'dolphin', 'attachments');
};
...
const buildLogExportFileName = (): string => {
  ...
  return `${APP_NAME}-logs-${datePart}-${timePart}.zip`;
};
...
if (!gotTheLock) {
  ...
  const authSessionManager = new AuthSessionManager({
    ...
    fetch: (url, options) => net.fetch(url, options),
    getRefreshUrl: () => `${getAuthApiBaseUrl()}/api/auth/refresh`,
  ...
  const loadAvailableServerModels = async (options: {
    ...
    ), undefined, buildServerModelCapabilityHeaders(app.getVersion()));
    console.log(
  ...
  const clearLocalAuthSession = (options: {
    ...
    const webSessionCleanup = clearAuthWebSessionStorage().catch(error => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const tmpDir = path.join(app.getPath('temp'), 'dolphin-thumbnails');
  ...
  ipcMain.handle(ShellIpc.OpenHtmlInBrowser, async (_event, htmlContent: string) => {
    try {
      const tmpDir = path.join(os.tmpdir(), 'dolphin-preview');
...
<body>
  ...
  const initApp = async () => {
    ...
    const defaultProjectDir = path.join(os.homedir(), 'dolphin', 'project');
