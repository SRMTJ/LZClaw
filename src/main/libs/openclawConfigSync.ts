...
const MANAGED_WEB_SEARCH_POLICY_PROMPT = [
  ...
  '- Only use the 海豚买买AI工作台 `web-search` skill when local command execution is available. Native channel sessions may deny `exec`, so prefer `browser` or `web_fetch` there.',
  ...
  '',
  'Do not claim you searched the web unless you actually used `browser`, `web_fetch`, or the 海豚买买AI工作台 `web-search` skill.',
].join('\n');
...
const MANAGED_BROWSER_POLICY_PROMPT = [
  '## Browser Policy',
  '',
  '海豚买买AI工作台 does not support sandbox browser execution in this version.',
  '- For every `browser` tool call, set `target="host"` explicitly.',
  '- Do not use `target="sandbox"` or `target="node"` unless a future 海豚买买AI工作台 version explicitly enables it.',
...
const buildManagedSkillCreationPrompt = (skillsDirPath: string): string => [
  '## Skill Creation',
  '',
  'When the user asks you to create a new skill, you MUST place it under the 海豚买买AI工作台 skills directory:',
  '',
