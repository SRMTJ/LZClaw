...
const buildSkinPackInstruction = (selection?: CoworkMediaSelection): string => {
  ...
  const lines = [
    ...
    'The presentation may style only allow-listed 海豚买买AI工作台 surfaces and title bars. Do not choose a color theme ID: 海豚买买AI工作台 derives the preferred light or dark appearance from the validated palette and applies it through the existing theme system. Do not change page layout, component positions, or system icons.',
  ];
...
export const buildMediaGenerationTurnInstruction = (
  ...
  if (!selection || selection.mode === 'none') {
    if (hasMediaSkillActive) {
      return [
        '[海豚买买AI工作台 media generation tools - NOT AVAILABLE]',
  ...
  const lines = [
    '[海豚买买AI工作台 media generation turn instruction]',
    'The user selected a 海豚买买AI工作台 media generation model for this turn.',
    'IMPORTANT: Do NOT read or use the "seedance" or "seedream" skills for this request.',
    'The 海豚买买AI工作台 media generation tools (lobsterai_image_generate / lobsterai_video_generate) replace those skills when a media model is selected.',
  ...
  if (!selection.imageModelId && !selection.videoModelId && selection.modelId?.trim()) {
    lines.push(`You MUST use model "${selection.modelId.trim()}" for media generation. Do NOT use a different model unless the user explicitly requests a different 海豚买买AI工作台 media model by name.`);
  }
