...
describe('buildMediaGenerationTurnInstruction', () => {
  ...
  test('uses the native image route for a skin workflow without 海豚买买AI工作台 image selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
      undefined,
      false,
      SkinWorkflowKind.SkinPack,
    ...
    expect(instruction).not.toContain('海豚买买AI工作台 media generation tools - NOT AVAILABLE');
  });

  test('uses the 海豚买买AI工作台 image route for auto media selection', () => {
    const instruction = buildMediaGenerationTurnInstruction(
  ...
  test('preserves the media-skill fallback when 海豚买买AI工作台 tools are unavailable', () => {
    const instruction = buildMediaGenerationTurnInstruction(undefined, true);

    expect(instruction).toContain('海豚买买AI工作台 media generation tools - NOT AVAILABLE');
