...
; electron-builder falls back to package.json "name" when the Windows
; executable name contains non-ASCII characters. Keep the legacy package name
; for compatibility, but use the LZClaw-specific directory for fresh installs.
...
!macro stopLobsterAIProcesses
  ...
  CreateDirectory "${LZCLAW_WINDOWS_USER_DATA_DIR}"
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "LOBSTERAI_STOP_LOG_PATH", t "${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log")i'
  ...
  !ifndef BUILD_UNINSTALLER
    ...
    StrCmp $lobsterTargetProcessesStopStatus "success" TargetProcessesStopped
      ...
      ${If} ${Silent}
        Banner::destroy
      ${EndIf}
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update stopped before replacing the previous version because the old application processes could not be confirmed stopped. Please close ${PRODUCT_NAME} and retry. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      SetErrorLevel 2
      Quit
    TargetProcessesStopped:

    ; -- Backup user-created skills to AppData before extraction overwrites them --
    ; Copy non-bundled skills to %APPDATA%\htmmai\skills-backup\ so they are
    ...
    ClearErrors
    FileOpen $R0 "${LZCLAW_WINDOWS_USER_DATA_DIR}\skill-migrate.log" w
    ...
    System::Call 'Kernel32::SetEnvironmentVariable(t "LOBSTERAI_SKILL_BACKUP_ROOT", t "${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup")i'
    ...
    FileSeek $9 0 END
    !insertmacro GetTimestamp $8
    FileWrite $9 "$8 phase=skill-backup-complete attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsStatus exit=$R2 elapsed_ms=$5 backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    FileClose $9
    ...
    StrCmp $lobsterLegacySkillsStatus "legacy-backup-succeeded" 0 SkillBackupFailedAbort
      ...
      FileSeek $9 0 END
      !insertmacro GetTimestamp $8
      FileWrite $9 "$8 phase=skill-backup-manifest-postcheck-missing attempt_id=$lobsterInstallerAttemptId manifest=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId\backup-manifest.json$\r$\n"
      FileClose $9
    SkillBackupFailedAbort:
      ...
      ${If} ${Silent}
        Banner::destroy
      ${EndIf}
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update stopped because legacy user skills could not be safely inspected or backed up (status=$lobsterLegacySkillsStatus). The previous installation was not replaced. Please retry the update. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      SetErrorLevel 2
      Quit
    ...
    OldInstallRenameVerificationFailed:
      ...
      FileClose $9
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update stopped because the previous installation move could not be verified and automatic recovery did not complete. No recovery copy was deleted. Restart Windows before retrying. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      ${If} ${Silent}
        Banner::destroy
      ${EndIf}
      SetErrorLevel 3
    ...
    OldInstallRenameVerificationRestored:
      ...
      FileClose $9
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update stopped because the previous installation move could not be verified. The previous version was restored. Please retry the update. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      ${If} ${Silent}
        Banner::destroy
      ${EndIf}
      SetErrorLevel 2
...
!macro customBeforeRegistryAddInstallInfo
  ...
  ; Log file: %APPDATA%\htmmai\install-timing.log

  CreateDirectory "${LZCLAW_WINDOWS_USER_DATA_DIR}"
  FileOpen $2 "${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" a
  FileSeek $2 0 END
  !insertmacro GetTimestamp $8
  ...
  System::Call 'Kernel32::SetEnvironmentVariable(t "LOBSTERAI_EXTRACTOR_LOG", t "${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log")i'
  nsExec::ExecToLog '"$lobsterTrustedPowerShellPath" -NoProfile -NonInteractive -Command "\
    $$ErrorActionPreference = \"Stop\";\
  ...
  MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because resource extraction completed without the required AI runtime entry. The installer will not commit a partial application. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
  ...
  TarExtractProcessFailed:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because the resource extractor could not be started (exit=$R2). The installer will not commit a partial application. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractTimeout:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because resource extraction timed out after 10 minutes. The blocked extractor was terminated and the installer will not commit a partial application. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractTerminationFailed:
    ...
    ${If} ${Silent}
      Banner::destroy
    ${EndIf}
    MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because the extractor process could not be confirmed terminated. No automatic rollback or cleanup was attempted while that process may still be writing files. Restart Windows before retrying. Recovery files (if any): $lobsterOldInstallBackupPath. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    SetErrorLevel 3
    Quit
  ...
  TarExtractOutputValidationFatal:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because the resource extractor watchdog returned an invalid result. The installer will not commit a partial application. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  TarExtractNonZero:
    ...
    FileClose $2
    MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because resource extraction failed (child exit code $R2). The installer will not commit a partial application. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    Goto TarExtractFailed
  ...
  StrCmp $lobsterOldInstallRollbackStatus "failed" 0 TarExtractAbort
    MessageBox MB_OK|MB_ICONEXCLAMATION "The installation failed and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
  TarExtractAbort:
  ${If} ${Silent}
    Banner::destroy
  ${EndIf}
  ...
  SkillRestoreAttemptBackupReady:
    ...
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
    FileWrite $2 "$8 phase=skill-restore-start attempt_id=$lobsterInstallerAttemptId backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    FileClose $2
    ...
    FileSeek $2 0 END
    !insertmacro GetTimestamp $8
    FileWrite $2 "$8 phase=skill-restore-complete attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus exit=$R2 elapsed_ms=$5 backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
    ...
    StrCmp $R2 "20" SkillRestoreConflictPreserved
      ...
      StrCmp $lobsterOldInstallRollbackStatus "success" SkillRestoreRollbackSucceeded
        MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update could not restore user skills, and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
        Goto SkillRestoreAbort
      SkillRestoreRollbackSucceeded:
        MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update could not restore user skills, so the previous version was restored. Please retry the update. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      SkillRestoreAbort:
      ${If} ${Silent}
        Banner::destroy
      ${EndIf}
    ...
    SkillRestoreFailurePreserved:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-degraded attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus action=continue-with-attempt-backup-preserved backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
      MessageBox MB_OK|MB_ICONEXCLAMATION "${PRODUCT_NAME} will finish installing, but legacy user skills could not be restored automatically ($lobsterLegacySkillsRestoreStatus). The recovery backup was preserved at ${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
    ...
    SkillRestoreDegradedBackupMissing:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-degraded attempt_id=$lobsterInstallerAttemptId status=$lobsterLegacySkillsRestoreStatus action=continue-no-backup-found backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
      MessageBox MB_OK|MB_ICONEXCLAMATION "${PRODUCT_NAME} will finish installing, but the recovery backup for legacy user skills was not found, so no skills were restored ($lobsterLegacySkillsRestoreStatus). Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      Goto SkillRestoreValidated
    ...
    SkillRestoreConflictPreserved:
      ...
      FileSeek $2 0 END
      !insertmacro GetTimestamp $8
      FileWrite $2 "$8 phase=skill-restore-conflict-preserved attempt_id=$lobsterInstallerAttemptId status=name-conflict action=attempt-backup-preserved backup=${LZCLAW_WINDOWS_USER_DATA_DIR}\skills-backup\$lobsterInstallerAttemptId$\r$\n"
      FileClose $2
  ...
  NewInstallPrevalidateFailed:
    ...
    StrCmp $lobsterOldInstallRollbackStatus "success" NewInstallPrevalidateRollbackSucceeded
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update could not be validated, and automatic rollback did not complete. No recovery copy was deleted. Previous files: $lobsterOldInstallBackupPath. Partial update: $lobsterOldInstallFailedPath. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      Goto NewInstallPrevalidateAbortAfterMessage
    NewInstallPrevalidateRollbackSucceeded:
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} update could not be validated, so the previous version was restored. Please retry the update. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
      Goto NewInstallPrevalidateAbortAfterMessage
    NewInstallPrevalidateAbort:
      MessageBox MB_OK|MB_ICONEXCLAMATION "The ${PRODUCT_NAME} installation stopped because the new application could not be validated ($lobsterNewInstallValidationReason). New registration and shortcuts were not written. Details: ${LZCLAW_WINDOWS_USER_DATA_DIR}\install-timing.log" /SD IDOK
