...
COMMAND_MAP = {'rate': 'claw_lead_rating_tagging.py', 'tag': 'claw_lead_rating_tagging.py', 'enrich': 'claw_lead_rating_tagging.py'}
DEFAULT_SUBCOMMAND = 'rate'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-lead-rating-tagging] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-lead-rating-tagging] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-lead-rating-tagging] supported subcommands: {commands}', file=sys.stderr)
