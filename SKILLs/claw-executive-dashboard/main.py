...
COMMAND_MAP = {'dashboard': 'claw_executive_dashboard.py', 'daily': 'claw_executive_dashboard.py', 'weekly': 'claw_executive_dashboard.py'}
DEFAULT_SUBCOMMAND = 'dashboard'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-executive-dashboard] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-executive-dashboard] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-executive-dashboard] supported subcommands: {commands}', file=sys.stderr)
