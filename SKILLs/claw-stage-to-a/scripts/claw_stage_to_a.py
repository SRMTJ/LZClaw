...
CONFIG = {
  ...
  "skill_name": "LZClaw-B到A推进",
  "skill_dir": "claw-stage-to-a",
  "default_mode": "stage-check",
  "sources": [
    "customer_detail",
    "ai_records"
  ],
  ...
  "interface_usage": [
    ...
    {
      "endpoint": "GET /async-tasks/{id}",
      "purpose": "查询异步任务状态。",
      "timing": "当写接口返回异步任务 ID，需要查看处理进度或结果时调用。",
      "params": "`id` 来自前置 POST 接口返回的任务 ID。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    }
  ],
  "primary": "ai_records",
  "judgment": "推进到 A 前必须确认合同、版本、费用和付款时间。",
  "next_action": "校验通过后确认阶段推进，并交给合同催收。",
  "handoff": "合同催收专员·陆款 Agent",
  "checks": [
    "目标阶段 A",
    "付款条件",
    "合同信息",
