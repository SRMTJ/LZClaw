...
COMMAND_MAP = {'forecast': 'claw_performance_forecast.py', 'target': 'claw_performance_forecast.py', 'amount': 'claw_performance_forecast.py'}
DEFAULT_SUBCOMMAND = 'forecast'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-performance-forecast] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-performance-forecast] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-performance-forecast] supported subcommands: {commands}', file=sys.stderr)
