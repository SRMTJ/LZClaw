...
COMMAND_MAP = {'team-review': 'claw_team_process_review.py', 'process': 'claw_team_process_review.py', 'users': 'claw_team_process_review.py'}
DEFAULT_SUBCOMMAND = 'team-review'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-team-process-review] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-team-process-review] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-team-process-review] supported subcommands: {commands}', file=sys.stderr)
