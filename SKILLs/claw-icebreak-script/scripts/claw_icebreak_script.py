...
CONFIG = {
  ...
  "skill_name": "LZClaw-破冰话术生成",
  "skill_dir": "claw-icebreak-script",
  "default_mode": "script",
  "sources": [
    "leads",
    "lead_detail"
  ...
  "interface_usage": [
    ...
    {
      "endpoint": "POST /leads/{lead_id}/script",
      "purpose": "生成线索推荐破冰话术。",
      "timing": "当用户需要电话、微信、短信或私信首触话术时调用。",
      "params": "`lead_id` 来自用户输入或 `GET /my-leads` 的返回结果。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    }
  ],
  "primary": "leads",
  "judgment": "话术需要贴近客户行业和线索来源。",
  "next_action": "生成首触话术后，设计完整触达路径。",
  "handoff": "LZClaw-触达路径设计",
  "checks": [
    "客户背景",
    "触达渠道",
    "行业痛点",
