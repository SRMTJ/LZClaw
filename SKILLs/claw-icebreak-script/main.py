...
COMMAND_MAP = {'script': 'claw_icebreak_script.py', 'phone': 'claw_icebreak_script.py', 'wechat': 'claw_icebreak_script.py'}
DEFAULT_SUBCOMMAND = 'script'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-icebreak-script] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-icebreak-script] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-icebreak-script] supported subcommands: {commands}', file=sys.stderr)
