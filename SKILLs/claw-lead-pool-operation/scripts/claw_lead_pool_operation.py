...
CONFIG = {
  ...
  "skill_name": "LZClaw-线索公海操作",
  "skill_dir": "claw-lead-pool-operation",
  "default_mode": "pool-list",
  "sources": [
    "lead_pool",
    "assignable_users"
  ],
  ...
  "interface_usage": [
    {
      "endpoint": "GET /lead-pool",
      "purpose": "读取线索公海列表，用于查看待认领、待分配或待筛选线索。",
      "timing": "当用户需要处理线索公海、认领线索或分配线索时调用。",
      "params": "筛选条件来自用户请求；缺省使用默认分页。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "GET /lead-pool/{lead_id}",
      "purpose": "读取线索公海详情，用于确认公海线索是否值得认领或分配。",
      "timing": "当用户指定公海线索或列表结果需要进一步核验时调用。",
      "params": "`lead_id` 来自用户输入或 `GET /lead-pool` 的返回结果。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "GET /assignable-users",
      "purpose": "读取可分配用户列表，用于线索/客户分配、转移或主管介入。",
      "timing": "当技能需要选择负责人、协作人或分配对象时调用。",
      "params": "分页或角色筛选来自用户请求；缺省使用默认分页。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "POST /lead-pool",
      "purpose": "手动创建公海线索，用于把用户提供的线索写入 CRM 公海。",
      "timing": "仅在用户明确要求创建公海线索，并提供必要字段后调用。",
      "params": "线索名称、行业、地区、联系方式等来自用户输入或已确认的搜索结果。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
      "endpoint": "POST /lead-pool/{lead_id}/claim",
      "purpose": "认领公海线索，将线索归入当前账号可跟进范围。",
      "timing": "仅在用户明确要求认领指定公海线索时调用。",
      "params": "`lead_id` 来自用户输入或 `GET /lead-pool` 的返回结果。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
      "endpoint": "POST /lead-pool/{lead_id}/assign",
      "purpose": "分配公海线索给指定销售或用户。",
      "timing": "仅在用户明确要求分配线索，并确认接收人后调用。",
      "params": "`lead_id` 来自公海线索；`target_user_id` 来自 `GET /assignable-users`。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    }
  ],
  "primary": "lead_pool",
  "judgment": "公海线索操作会改变归属，必须先确认对象和动作。",
  "next_action": "认领后进入我的线索筛选和破冰建联。",
  "handoff": "LZClaw-CRM线索筛选",
  "checks": [
    "公海线索是否重复",
    "认领/分配对象是否明确",
    "是否具备跟进价值"
