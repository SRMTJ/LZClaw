...
COMMAND_MAP = {'version': 'claw_version_quote_strategy.py', 'quote': 'claw_version_quote_strategy.py', 'value': 'claw_version_quote_strategy.py'}
DEFAULT_SUBCOMMAND = 'version'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-version-quote-strategy] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-version-quote-strategy] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-version-quote-strategy] supported subcommands: {commands}', file=sys.stderr)
