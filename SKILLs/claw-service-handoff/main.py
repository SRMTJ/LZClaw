...
COMMAND_MAP = {'handoff': 'claw_service_handoff.py', 'stage-check': 'claw_service_handoff.py', 'stage-confirm': 'claw_service_handoff.py'}
DEFAULT_SUBCOMMAND = 'handoff'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-service-handoff] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-service-handoff] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-service-handoff] supported subcommands: {commands}', file=sys.stderr)
