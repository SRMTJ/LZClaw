...
CONFIG = {
  "agent_name": "合同催收专员·陆款 Agent",
  "skill_name": "LZClaw-服务群衔接",
  "skill_dir": "claw-service-handoff",
  ...
  "interface_usage": [
    {
      "endpoint": "POST /customers/{customer_id}/stage-transitions",
      "purpose": "执行 AI 阶段推进校验。",
      "timing": "当用户需要判断客户是否能推进到目标阶段时调用。",
      "params": "`customer_id` 来自客户；`target_stage` 来自用户请求或技能目标。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
      "endpoint": "POST /customers/{customer_id}/stage-transitions/confirm",
      "purpose": "确认客户阶段推进。",
      "timing": "仅在阶段校验通过且用户明确确认推进后调用。",
      "params": "`customer_id` 和 `target_stage` 来自校验结果及用户确认。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
  ...
  "judgment": "推进 S 阶段必须有付款或签约确认。",
  "next_action": "进入 S 阶段后交给服务或经营复盘。",
