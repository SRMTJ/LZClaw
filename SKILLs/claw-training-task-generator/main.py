...
COMMAND_MAP = {'training-task': 'claw_training_task_generator.py', 'roleplay': 'claw_training_task_generator.py', 'checklist': 'claw_training_task_generator.py'}
DEFAULT_SUBCOMMAND = 'training-task'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-training-task-generator] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-training-task-generator] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-training-task-generator] supported subcommands: {commands}', file=sys.stderr)
