...
COMMAND_MAP = {'decision-chain': 'claw_decision_chain_analysis.py', 'stakeholder': 'claw_decision_chain_analysis.py', 'approval-risk': 'claw_decision_chain_analysis.py'}
DEFAULT_SUBCOMMAND = 'decision-chain'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-decision-chain-analysis] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-decision-chain-analysis] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-decision-chain-analysis] supported subcommands: {commands}', file=sys.stderr)
