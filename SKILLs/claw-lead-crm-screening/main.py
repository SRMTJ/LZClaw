...
COMMAND_MAP = {'screen': 'claw_lead_crm_screening.py', 'judge-leads': 'claw_lead_crm_screening.py', 'lead-detail': 'claw_lead_crm_screening.py'}
DEFAULT_SUBCOMMAND = 'screen'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-lead-crm-screening] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-lead-crm-screening] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-lead-crm-screening] supported subcommands: {commands}', file=sys.stderr)
