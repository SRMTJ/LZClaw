...
COMMAND_MAP = {'objection': 'claw_objection_handling.py', 'reply': 'claw_objection_handling.py', 'next-step': 'claw_objection_handling.py'}
DEFAULT_SUBCOMMAND = 'objection'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-objection-handling] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-objection-handling] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-objection-handling] supported subcommands: {commands}', file=sys.stderr)
