...
COMMAND_MAP = {
    'scan-headers': 'scan_headers.py',
    'wyw-check': 'wyw_check.py',
}
DEFAULT_SUBCOMMAND = 'scan-headers'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[csm-data-audit] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[csm-data-audit] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[csm-data-audit] supported subcommands: {commands}', file=sys.stderr)
