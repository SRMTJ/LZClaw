...
COMMAND_MAP = {'stage-check': 'claw_solution_stage_advance.py', 'stage-confirm': 'claw_solution_stage_advance.py', 'advance': 'claw_solution_stage_advance.py'}
DEFAULT_SUBCOMMAND = 'stage-check'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-solution-stage-advance] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-solution-stage-advance] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-solution-stage-advance] supported subcommands: {commands}', file=sys.stderr)
