...
COMMAND_MAP = {'funnel': 'claw_funnel_diagnosis.py', 'conversion': 'claw_funnel_diagnosis.py', 'bottleneck': 'claw_funnel_diagnosis.py'}
DEFAULT_SUBCOMMAND = 'funnel'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-funnel-diagnosis] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-funnel-diagnosis] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-funnel-diagnosis] supported subcommands: {commands}', file=sys.stderr)
