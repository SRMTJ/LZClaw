...
COMMAND_MAP = {'countdown': 'claw_contract_payment_plan.py', 'create-plan': 'claw_contract_payment_plan.py', 'save-record': 'claw_contract_payment_plan.py'}
DEFAULT_SUBCOMMAND = 'countdown'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-contract-payment-plan] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-contract-payment-plan] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-contract-payment-plan] supported subcommands: {commands}', file=sys.stderr)
