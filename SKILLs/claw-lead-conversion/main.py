...
COMMAND_MAP = {'convert': 'claw_lead_conversion.py', 'task-status': 'claw_lead_conversion.py', 'transfer': 'claw_lead_conversion.py'}
DEFAULT_SUBCOMMAND = 'convert'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-lead-conversion] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-lead-conversion] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-lead-conversion] supported subcommands: {commands}', file=sys.stderr)
