...
CONFIG = {
  ...
  "skill_name": "LZClaw-线索转客户",
  "skill_dir": "claw-lead-conversion",
  ...
  "interface_usage": [
    {
      "endpoint": "POST /leads/{lead_id}/convert-to-customer",
      "purpose": "将线索转为客户。",
      "timing": "仅在转客户校验通过且用户确认转换后调用。",
    ...
    {
      "endpoint": "POST /leads/{lead_id}/transfer-records",
      "purpose": "转移线索负责人或相关记录。",
      "timing": "仅在用户明确要求转移线索并确认目标负责人后调用。",
      "params": "`lead_id` 来自线索；`target_user_id` 来自可分配用户。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    }
  ],
  "primary": "assignable_users",
  "judgment": "仅在转客户校验通过且用户确认后执行转换。",
  "next_action": "转换成功后交给客情诊断。",
  "handoff": "客情诊断师·闻策 Agent",
  "checks": [
    "转客户校验结果",
    "负责人",
    "线索状态",
