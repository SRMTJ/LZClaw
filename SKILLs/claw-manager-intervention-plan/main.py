...
COMMAND_MAP = {'intervention': 'claw_manager_intervention_plan.py', 'coaching': 'claw_manager_intervention_plan.py', 'create-plan': 'claw_manager_intervention_plan.py'}
DEFAULT_SUBCOMMAND = 'intervention'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-manager-intervention-plan] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-manager-intervention-plan] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-manager-intervention-plan] supported subcommands: {commands}', file=sys.stderr)
