...
CONFIG = {
  ...
  "skill_name": "LZClaw-主管介入与辅导",
  "skill_dir": "claw-manager-intervention-plan",
  "default_mode": "intervention",
  "sources": [
    "customer_detail",
    "users",
    "assignable_users"
  ...
  "interface_usage": [
    ...
    {
      "endpoint": "POST /customers/{customer_id}/follow-up-plans",
      "purpose": "创建客户跟进计划。",
      "timing": "仅在用户明确要求创建计划、倒排节点或主管督办计划时调用。",
      "params": "`customer_id` 来自客户；计划内容、时间和责任人来自用户确认。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    }
  ],
  "primary": "users",
  "judgment": "主管介入要避免空泛，明确客户和销售动作。",
  "next_action": "介入方案纳入团队过程复盘。",
  "handoff": "LZClaw-团队过程复盘",
  "checks": [
    "介入理由",
    "介入对象",
    "主管话术",
