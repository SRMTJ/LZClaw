...
COMMAND_MAP = {'risk': 'claw_risk_resource_advice.py', 'resource': 'claw_risk_resource_advice.py', 'boss-action': 'claw_risk_resource_advice.py'}
DEFAULT_SUBCOMMAND = 'risk'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-risk-resource-advice] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-risk-resource-advice] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-risk-resource-advice] supported subcommands: {commands}', file=sys.stderr)
