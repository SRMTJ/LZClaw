...
COMMAND_MAP = {'stage-check': 'claw_customer_stage_check.py', 'stage-confirm': 'claw_customer_stage_check.py', 'ai-records': 'claw_customer_stage_check.py'}
DEFAULT_SUBCOMMAND = 'stage-check'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-customer-stage-check] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-customer-stage-check] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-customer-stage-check] supported subcommands: {commands}', file=sys.stderr)
