...
COMMAND_MAP = {'case-review': 'claw_case_review.py', 'success-case': 'claw_case_review.py', 'failure-case': 'claw_case_review.py'}
DEFAULT_SUBCOMMAND = 'case-review'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-case-review] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-case-review] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-case-review] supported subcommands: {commands}', file=sys.stderr)
