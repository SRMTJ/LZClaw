...
COMMAND_MAP = {'stagnation': 'claw_stage_stagnation_risk.py', 'risk': 'claw_stage_stagnation_risk.py', 'priority': 'claw_stage_stagnation_risk.py'}
DEFAULT_SUBCOMMAND = 'stagnation'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-stage-stagnation-risk] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-stage-stagnation-risk] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-stage-stagnation-risk] supported subcommands: {commands}', file=sys.stderr)
