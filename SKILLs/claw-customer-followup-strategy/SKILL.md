...
~~~bash
python "$SKILLS_ROOT/claw-customer-followup-strategy/main.py" plan
~~~
...
- `POST /customers/{customer_id}/follow-up-summary-drafts`
  - 用途：生成客户跟进总结草稿。
  - 调用时机：当用户需要整理客户沟通、拜访或电话纪要时调用。
  - 参数来源：`customer_id` 来自客户；沟通内容来自用户输入或 CRM 上下文。
  - 安全边界：POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。
