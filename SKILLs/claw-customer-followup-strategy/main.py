...
COMMAND_MAP = {'plan': 'claw_customer_followup_strategy.py', 'summary-draft': 'claw_customer_followup_strategy.py', 'save-record': 'claw_customer_followup_strategy.py'}
DEFAULT_SUBCOMMAND = 'plan'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-customer-followup-strategy] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-customer-followup-strategy] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-customer-followup-strategy] supported subcommands: {commands}', file=sys.stderr)
