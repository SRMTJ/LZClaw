...
COMMAND_MAP = {'second-meeting': 'claw_second_meeting_strategy.py', 'confirm-questions': 'claw_second_meeting_strategy.py', 'quote-prep': 'claw_second_meeting_strategy.py'}
DEFAULT_SUBCOMMAND = 'second-meeting'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-second-meeting-strategy] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-second-meeting-strategy] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-second-meeting-strategy] supported subcommands: {commands}', file=sys.stderr)
