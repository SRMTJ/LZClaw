...
COMMAND_MAP = {'contract-script': 'claw_contract_payment_script.py', 'payment-reminder': 'claw_contract_payment_script.py', 'group-script': 'claw_contract_payment_script.py'}
DEFAULT_SUBCOMMAND = 'contract-script'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-contract-payment-script] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-contract-payment-script] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-contract-payment-script] supported subcommands: {commands}', file=sys.stderr)
