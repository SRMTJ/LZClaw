...
COMMAND_MAP = {'score': 'claw_sales_process_scoring.py', 'process-review': 'claw_sales_process_scoring.py', 'quality': 'claw_sales_process_scoring.py'}
DEFAULT_SUBCOMMAND = 'score'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-sales-process-scoring] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-sales-process-scoring] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-sales-process-scoring] supported subcommands: {commands}', file=sys.stderr)
