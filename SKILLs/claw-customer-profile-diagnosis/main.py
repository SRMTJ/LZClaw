...
COMMAND_MAP = {'profile': 'claw_customer_profile_diagnosis.py', 'diagnose': 'claw_customer_profile_diagnosis.py', 'customer-detail': 'claw_customer_profile_diagnosis.py'}
DEFAULT_SUBCOMMAND = 'profile'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-customer-profile-diagnosis] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-customer-profile-diagnosis] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-customer-profile-diagnosis] supported subcommands: {commands}', file=sys.stderr)
