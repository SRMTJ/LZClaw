...
CONFIG = {
  "agent_name": "破冰建联官·许开 Agent",
  "skill_name": "LZClaw-线索跟进总结",
  "skill_dir": "claw-lead-followup-summary",
  "default_mode": "summary-draft",
  "sources": [
    "lead_detail"
  ],
  "dict_types": [],
  ...
  "interface_usage": [
    {
      "endpoint": "POST /leads/{lead_id}/follow-up-summary-drafts",
      "purpose": "生成线索跟进总结草稿。",
      "timing": "当用户提供沟通内容并需要整理跟进总结时调用。",
      "params": "`lead_id` 来自线索；沟通内容来自用户输入或 CRM 跟进上下文。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
      "endpoint": "POST /leads/{lead_id}/follow-ups",
      "purpose": "保存线索跟进记录，并触发转客户校验链路。",
      "timing": "仅在用户明确要求保存线索跟进记录时调用。",
      "params": "`lead_id` 来自线索；跟进内容、下次动作来自用户确认。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
  ...
  "primary": "leads",
  "judgment": "保存跟进前必须确认记录内容准确。",
  "next_action": "跟进结果达到 D 阶段后，交给线索转客户。",
  "handoff": "LZClaw-线索转客户",
  "checks": [
    "沟通事实",
    "客户反馈",
    "下一步动作",
