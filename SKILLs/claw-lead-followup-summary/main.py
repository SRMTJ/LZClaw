...
COMMAND_MAP = {'summary-draft': 'claw_lead_followup_summary.py', 'save-followup': 'claw_lead_followup_summary.py', 'convert-check': 'claw_lead_followup_summary.py'}
DEFAULT_SUBCOMMAND = 'summary-draft'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-lead-followup-summary] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-lead-followup-summary] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-lead-followup-summary] supported subcommands: {commands}', file=sys.stderr)
