...
CONFIG = {
  ...
  "skill_name": "LZClaw-线索挖掘任务",
  "skill_dir": "claw-source-search-run",
  "default_mode": "create-search",
  "sources": [
    "search_runs",
    "search_results"
  ],
  ...
  "interface_usage": [
    {
      "endpoint": "POST /source-search-runs",
      "purpose": "发起高德线索挖掘任务。",
      "timing": "仅在用户确认城市、关键词和搜索目标后调用。",
      "params": "城市、关键词、行业、区域等来自用户输入和关键词策略结果。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    },
    {
      "endpoint": "GET /source-search-runs",
      "purpose": "读取线索挖掘任务列表，用于查看任务状态和历史挖掘记录。",
      "timing": "当用户询问挖掘任务进度或需要选择任务结果时调用。",
      "params": "分页、状态、时间范围来自用户请求；缺省使用默认分页。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "GET /source-search-runs/{id}/results",
      "purpose": "读取指定线索挖掘任务的结果列表。",
      "timing": "当挖掘任务完成后需要筛选、去重或入池时调用。",
      "params": "`id` 来自用户输入或 `GET /source-search-runs` 的返回结果。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "POST /source-search-results/join-pool",
      "purpose": "将挖掘结果加入线索公海。",
      "timing": "仅在用户确认要入池哪些挖掘结果后调用。",
      "params": "结果 ID 列表来自 `GET /source-search-runs/{id}/results` 的返回结果。",
      "safety": "POST 写操作，会改变 CRM 状态或生成业务记录，必须在用户明确要求并确认对象后才调用；脚本默认 dry-run，需 `--execute` 才执行。"
    }
  ],
  "primary": "search_runs",
  "judgment": "线索挖掘任务需要先看任务状态和结果质量。",
  "next_action": "筛选可入池结果，交给 CRM 线索筛选或公海认领。",
