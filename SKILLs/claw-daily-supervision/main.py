...
COMMAND_MAP = {'daily-supervision': 'claw_daily_supervision.py', 'today': 'claw_daily_supervision.py', 'notifications': 'claw_daily_supervision.py'}
DEFAULT_SUBCOMMAND = 'daily-supervision'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-daily-supervision] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-daily-supervision] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-daily-supervision] supported subcommands: {commands}', file=sys.stderr)
