...
COMMAND_MAP = {'route': 'claw_contact_route_design.py', 'call-wechat-flow': 'claw_contact_route_design.py', 'touch-plan': 'claw_contact_route_design.py'}
DEFAULT_SUBCOMMAND = 'route'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-contact-route-design] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-contact-route-design] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-contact-route-design] supported subcommands: {commands}', file=sys.stderr)
