...
CONFIG = {
  ...
  "skill_name": "LZClaw-地图关键词策略",
  "skill_dir": "claw-lead-keyword-strategy",
  "default_mode": "keywords",
  "sources": [
    "dictionaries"
  ],
  "dict_types": [
  ...
  "interface_usage": [
    {
      "endpoint": "GET /dictionaries?dict_type=sub_industry",
      "purpose": "读取细分行业字典，用于生成行业关键词、案例匹配和行业机会分析。",
      "timing": "当技能需要按行业细分扩展关键词或判断行业方向时调用。",
      "params": "`dict_type` 固定为 `sub_industry`。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    },
    {
      "endpoint": "GET /dictionaries?dict_type=city_adcode",
      "purpose": "读取城市和行政区划字典，用于生成地图搜索区域和区域机会分析。",
      "timing": "当技能需要把城市、区域或商圈转换成搜索条件时调用。",
      "params": "`dict_type` 固定为 `city_adcode`。",
      "safety": "只读接口，可直接用于分析，不修改 CRM 数据。"
    }
  ],
  "primary": "dictionaries",
  "judgment": "可作为地图搜索和线索挖掘关键词参考。",
  "next_action": "生成地图搜索关键词和排除词后，交给线索挖掘任务执行。",
  "handoff": "LZClaw-线索挖掘任务",
  "checks": [
    "行业不能过泛",
    "关键词按细分行业分组",
    "优先匹配目标客户画像"
