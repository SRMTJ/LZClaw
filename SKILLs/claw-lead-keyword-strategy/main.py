...
COMMAND_MAP = {'keywords': 'claw_lead_keyword_strategy.py', 'industry-dict': 'claw_lead_keyword_strategy.py', 'search-plan': 'claw_lead_keyword_strategy.py'}
DEFAULT_SUBCOMMAND = 'keywords'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-lead-keyword-strategy] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-lead-keyword-strategy] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-lead-keyword-strategy] supported subcommands: {commands}', file=sys.stderr)
