...
COMMAND_MAP = {'readiness': 'claw_deal_readiness.py', 'deal-check': 'claw_deal_readiness.py', 'risk': 'claw_deal_readiness.py'}
DEFAULT_SUBCOMMAND = 'readiness'


def print_usage() -> None:
    commands = ', '.join(COMMAND_MAP.keys())
    print('[claw-deal-readiness] missing or invalid subcommand, fallback to default', file=sys.stderr)
    print(f'[claw-deal-readiness] default subcommand: {DEFAULT_SUBCOMMAND}', file=sys.stderr)
    print(f'[claw-deal-readiness] supported subcommands: {commands}', file=sys.stderr)
